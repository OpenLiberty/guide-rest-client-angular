...
defined in the [hotspot]`app.module.ts` file. 

=== Importing the Angular HTTP client module

Begin by creating the root module which includes the Angular HTTP Client module.
...
The [hotspot=importHttpClientModule]`HttpClientModule` class into the file, then
...
== Creating a component to comsume the service
...
Create the [hotspot=appComponentClass]`AppComponent` class to use the artists service
